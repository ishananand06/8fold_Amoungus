...
class ReplayTheater:
    def __init__(self, log_path):
        ...
        self.play_speed = 1.0  # Seconds per round
        self.last_round_time = 0
        
        # Animation state
        self.player_lerp = 1.0 # 0.0 to 1.0 progress through round movement
        self.prev_positions = {}
    ...
    def update_animation_targets(self):
        # Set prev to current target
        self.prev_positions = self.target_positions.copy()
        
        # Get next targets from log
    ...
    def draw_map(self):
        ...
        for room, neighbors in ADJACENCY.items():
            ...
            for n in neighbors:
                end = ROOM_POSITIONS[n]
                pygame.draw.line(self.screen, COLORS["border"], start, end, 2)
        ...
        for name, pos in ROOM_POSITIONS.items():
            ...
            if name in active_sab_rooms:
                ...
                self.draw_rounded_rect(glow_rect, COLORS["sabotage"], width=2)
            ...
            # Bodies
            bodies_in_room = [b for b in state.get("bodies", []) if b["location"] == name]
            if bodies_in_room:
    ...
    def draw_players(self):
        ...
        for pid in self.target_positions.keys():
            ...
            # Draw player sprite (Circle + Backpack)
            draw_color = color if is_alive else COLORS["dead"]
            pygame.draw.rect(self.screen, draw_color, (curr_x-18, curr_y-8, 10, 16)) # Backpack
    ...
    def draw_ui(self):
        ...
        # Round info
        rnd_text = self.font_main.render(f"ROUND {self.current_round_idx:02d}", True, COLORS["white"])
        self.screen.blit(rnd_text, (20, 20))
        
        # Current events ticker
        log = self.game_log[self.current_round_idx]
    ...
    def draw_meeting(self, meeting):
        ...
        title = self.font_main.render("--- EMERGENCY MEETING ---", True, COLORS["impostor"])
        self.screen.blit(title, (SCREEN_WIDTH//2 - title.get_width()//2, 100))
        
        y = 160
        for msg in meeting.get("transcript", [])[-10:]: # Show last 10 messages
            color = COLORS["crewmate"] if self.all_roles.get(msg['speaker']) == "crewmate" else COLORS["impostor"]
            txt = self.font_small.render(f"{msg['speaker']}: {msg['message'][:80]}...", True, color)
            self.screen.blit(txt, (SCREEN_WIDTH//2 - 400, y))
            y += 25
            
        res_text = f"RESULT: {meeting['voted_out']} EJECTED" if meeting['voted_out'] else "RESULT: SKIP"
        res_render = self.font_main.render(res_text, True, COLORS["sabotage"])
        self.screen.blit(res_render, (SCREEN_WIDTH//2 - res_render.get_width()//2, 550))
    ...
    def run_theater(self):
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_EQUALS: # Plus
                        self.play_speed = max(0.1, self.play_speed - 0.2)
            ...
            # Check for meetings (meetings occur AFTER the round in the log)
            meeting = next((m for m in self.meeting_history if m["round_called"] == self.current_round_idx + 1), None)
            if meeting and self.player_lerp >= 1.0:
                self.draw_meeting(meeting)
                if self.is_playing:
                    # Pause longer during meetings
                    self.last_round_time = time.time() + 2.0 
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python replay_theater.py <path_to_log.json>")
    else:
